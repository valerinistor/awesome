...
--  calendar.register(mytextclock, "<b>%s</b>") -- now the current day is bold instead of underlined
--
-- # How to Use #
...
-- Orginal: https://github.com/Mic92/utils/blob/master/cal.lua
...
function calendar.register(calendarWidget, custom_current_day_format)
    ...
    calendarWidget:connect_signal('mouse::leave', function () naughty.destroy(calendar[3]) end)

    calendarWidget:buttons(awful.util.table.join(
...
function switchMonth(delta)
    ...
    naughty.notify({
            text = string.format('<span font_desc="monospace">%s</span>', displayMonth(state[1], state[2], 2)),
            timeout = 0,
            hover_timeout = 0.5,
            screen = mouse.screen
        })
